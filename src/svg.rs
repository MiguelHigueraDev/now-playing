...
const ART_SIZE: u32 = 72;
const ART_RX: u32 = 10;
const CARD_RX: u32 = 14;
const TEXT_GAP: u32 = 16;
const BAR_H: u32 = 4;
const BAR_GAP_ABOVE: u32 = 10;
const TIME_LABEL_OFFSET: u32 = 12;
const BAR_Y: u32 = HEIGHT - PADDING - TIME_LABEL_OFFSET - BAR_H;
const BAR_WIDTH: u32 = WIDTH - PADDING * 2;
const TEXT_X: u32 = PADDING + ART_SIZE + TEXT_GAP;
const FONT_SANS: &str =
    "ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
/// Bottom edge of the content row (album art + metadata), above the progress bar.
const CONTENT_BOTTOM: u32 = BAR_Y - BAR_GAP_ABOVE;

const TRACK_FONT: u32 = 18;
const ARTIST_FONT: u32 = 13;
const ALBUM_FONT: u32 = 11;
const IDLE_MESSAGE_FONT: u32 = 14;
...
/// Baselines for track, artist, and album — vertically centered in the content row.
fn metadata_baselines() -> (u32, u32, u32) {
    let region_mid = (PADDING + CONTENT_BOTTOM) / 2;
...
pub fn render_now_playing_svg(input: SvgRenderInput<'_>) -> String {
    ...
    let track = truncate(&input.now_playing.track_name, 36);
    let artist = truncate(&input.now_playing.artist_name, 40);
    let album = truncate(&input.now_playing.album_name, 40);
    ...
    let art_foreground = artwork_foreground(&art_image, ART_SIZE, art_y);
...
{bg_gradient_defs}    <linearGradient id="art-shine" x1="0" y1="0" x2="0" y2="1">
    ...
    <clipPath id="art-clip">
      <rect x="{PADDING}" y="{art_y}" width="{ART_SIZE}" height="{ART_SIZE}" rx="{ART_RX}"/>
    </clipPath>
...
{bg_markup}
    <g filter="url(#art-shadow)" clip-path="url(#art-clip)">
      {art_foreground}
      <rect x="{PADDING}" y="{art_y}" width="{ART_SIZE}" height="{ART_SIZE}" fill="url(#art-shine)" pointer-events="none"/>
    </g>
    <rect x="{PADDING}" y="{art_y}" width="{ART_SIZE}" height="{ART_SIZE}" rx="{ART_RX}" fill="none" stroke="#ffffff" stroke-opacity="0.1" stroke-width="1"/>
    <rect x="{PADDING}" y="{art_y}" width="{ART_SIZE}" height="{ART_SIZE}" rx="{ART_RX}" fill="none" stroke="#000000" stroke-opacity="0.35" stroke-width="1" transform="translate(0 1)"/>
...
{bg_markup}
    ...
    <rect x="{PADDING}" y="{BAR_Y}" width="{BAR_WIDTH}" height="{BAR_H}" rx="2" fill="#ffffff" fill-opacity="0.08"/>
    <text x="{PADDING}" y="{time_label_y}" fill="#a39e94" font-family="{FONT_SANS}" font-size="10" font-variant-numeric="tabular-nums" letter-spacing="0.04em">0:00</text>
    <text x="{duration_label_x}" y="{time_label_y}" fill="#a39e94" font-family="{FONT_SANS}" font-size="10" font-variant-numeric="tabular-nums" letter-spacing="0.04em" text-anchor="end">0:00</text>
  </g>
  <rect width="{WIDTH}" height="{HEIGHT}" rx="{CARD_RX}" fill="none" stroke="#ffffff" stroke-opacity="0.07" stroke-width="1"/>
</svg>"##,
    )
...
fn artwork_foreground(href: &Option<String>, size: u32, y: u32) -> String {
    let Some(href) = href else {
        let center_x = PADDING + size / 2;
        let center_y = y + size / 2;
        return format!(
            r##"<rect x="{PADDING}" y="{y}" width="{size}" height="{size}" fill="#1a191e"/>
  <text x="{center_x}" y="{center_y}" fill="#5c584f" font-family="{FONT_SANS}" font-size="28" text-anchor="middle" dominant-baseline="middle">♪</text>"##
        );
    };

    format!(
        r##"<image x="{PADDING}" y="{y}" width="{size}" height="{size}" preserveAspectRatio="xMidYMid slice" href="{href}"/>"##
    )
}
